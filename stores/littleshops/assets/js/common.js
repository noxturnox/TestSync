...
function divWidthMenu() {
    ...
    if ($('.hd1').length || $('.hd2').length) {
        $('.home1 .container-fix .main-menu').outerWidth(
            width_count - width_bl - width_br - 30
        )
    }
}
...
$(document).ready(function() {
    ...
    if (sticky_menu == 1) {
        ...
        $('header').after(
            '<div class="headerSpace unvisible" style="height:' +
                headerSpaceH +
                'px;"></div>'
        )
    }
    ...
    $('#grid-view').click(function() {
        ...
        if (cols == 2) {
            $('#content .product-layout').attr(
                'class',
                'product-layout product-grid col-md-6 col-sm-6 col-xs-6 two-items'
            )
        } else if (cols == 1) {
            $('#content .product-layout').attr(
                'class',
                'product-layout product-grid col-md-4 col-sm-6 col-xs-6 three-items'
            )
        } else {
            $('#content .product-layout').attr(
                'class',
                'product-layout product-grid col-md-3 col-sm-6 col-xs-6 four-items'
            )
        }
    ...
    remove: function(key) {
        $.ajax({
            ...
            success: function(json) {
                ...
                if (
                    getURLVar('route') == '/cart' ||
                    getURLVar('route') == '/checkout'
                ) {
                    location = '/cart'
                } else {
                    $.get('/', function(data) {
