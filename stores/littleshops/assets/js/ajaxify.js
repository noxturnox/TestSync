...
Shopify.getProduct = function(handle, callback) {
    jQuery.getJSON('/products/' + handle + '.js', function(
        product,
        textStatus
    ) {
        if (typeof callback === 'function') {
...
var ajaxifyShopify = (function(module, $) {
  ...
  ==============================================================================*/
    ...
    flipSetup = function() {
        ...
        var checkoutBtn = $(
                '<a href="/cart" class="flip-back" style="background-color: #C00; color: #fff;" id="flip-checkout">' +
                    'Checkout' +
                    '</a>'
            ).addClass($btnClass),
            flipLoader = $('<span class="ajaxifyCart-loader"></span>'),
            flipExtra = $(
                '<div class="flip-extra">' +
                    'or' +
                    ' <a href="#" class="flip-cart">' +
                    'View Cart' +
                    ' (<span></span>)</a></div>'
            )
    ...
    modalSetup = function() {
        ...
        $modalContainer.prepend(
            '<button class="ajaxifyCart--close" title="' +
                'Close Cart' +
                '">' +
                'Close Cart' +
                '</button>'
    ...
    buildCart = function(cart) {
        ...
        $.each(cart.items, function(index, cartItem) {
            ...
            var prodVariation = cartItem.title
                .replace(/^[^\-]*/, '')
                .replace(/-/, '')
    ...
    adjustCart = function() {
        ...
        function updateQuantity(line, qty) {
            ...
            if (!settings.useCartTemplate) {
                var row = $(
                    '.ajaxifyCart--row[data-line="' + line + '"]'
                ).addClass('ajaxifyCart--is-loading')
            } else {
        ...
        if (typeof AmazonPaymentsPayButton === 'function')
