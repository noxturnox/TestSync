$(document).ready(function() {
    ...
    var main_image = $(
            '#productModal .modal-body .product-images .main-image img'
        ),
        main_href = $(
            '#productModal .modal-body .product-images .main-image a'
        ),
        other_image = $(
            '#productModal .modal-body .product-images .other-image  ul'
        ),
        ...
        btn_add = $(
            '#productModal .modal-body .product-info .quick-add-to-cart .single_add_to_cart_button'
        )

    field_add = $(
        '#productModal .modal-body .product-info .quick-add-to-cart #id'
    )

    btn_compare = $(
        '#productModal .modal-body .product-info .button-group .btn-compare'
    )

    btn_wishlist = $(
        '#productModal .modal-body .product-info .button-group .btn-wishlist'
    )
    ...
    $('body').on('click', '.quickview', function() {
        ...
        for (var i = 0; i < arr_image.length && i < 3; i++) {
            other_image.append(
                '<li><img style="width:78px;cursor:pointer" class="img-thumbnail other-image" src="' +
                    arr_image[i] +
                    '"></li>'
            )
        }
                    ...
                    price.append(
                        '<span class="new-price">' +
                            Currency.moneyFormats[
                                shopCurrency
                            ].money_format.substring(0, 1) +
                            product_info['price'] / 100 +
                            '</span><span class="old-price">' +
                            Currency.moneyFormats[
                                shopCurrency
                            ].money_format.substring(0, 1) +
                            product_info['compare_at_price'] / 100 +
                            '</span>'
                    )
                } else {
                    price.append(
        ...
        btn_compare.attr(
            'onclick',
            'compare.add(' + product_info['product_id'] + ')'
        )
        btn_wishlist.attr(
            'onclick',
            'wishlish.add(' + product_info['product_id'] + ')'
        )
    })
})
