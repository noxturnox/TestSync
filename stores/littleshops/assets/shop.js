...
timber.alignMenu = function() {
    var $nav = timber.cache.$navigation,
        w = 0,
        i = 0
    ;(wrapperWidth = $nav.outerWidth() - 101), (menuhtml = '')
    ...
    if (wrapperWidth < w) {
        $nav.append(
            '<li id="moreMenu" class="site-nav--has-dropdown">' +
                ...
                '<ul id="moreMenu--list" class="site-nav--dropdown">' +
                menuhtml +
                '</ul></li>'
        )
...
timber.toggleMenu = function() {
    ...
    timber.cache.$hasDropdownItem.on('click', function(evt) {
        ...
        if (
            (!$el.hasClass(showDropdownClass) && timber.vars.isTouch) ||
            (!$el.hasClass(showDropdownClass) &&
                timber.cache.$html.hasClass(showNavClass))
        ) {
            evt.preventDefault()
...
timber.socialSharing = function() {
    ...
    if ($fbLink.length) {
        $.getJSON(
            'https://graph.facebook.com/?id=' + permalink + '&callback=?',
            function(data) {
                if (data.shares) {
                    $fbLink
                        .find('.share-count')
...
timber.toggleFilters = function() {
    if (timber.cache.$collectionFilters.length) {
        timber.cache.$toggleFilterBtn.on('click', function() {
            ...
            if (
                $(window).scrollTop() > timber.cache.$breadcrumbs.offset().top
            ) {
                $('html, body').animate({
...
timber.formatMoney = function(val) {
    ...
    if (moneyFormat.indexOf('money') === -1) {
        if (
            moneyFormat.indexOf('{{amount}}') > -1 &&
            moneyFormat.indexOf('.') === -1
        ) {
            val = val.replace('.', '<sup>') + '</sup>'
        } else if (
            moneyFormat.indexOf('{{amount_with_comma_separator}}') > -1
        ) {
            val = val.replace(',', '<sup>') + '</sup>'
        }
    }
...
timber.formatSaleTag = function(val) {
    ...
    if (moneyFormat.indexOf('money') === -1) {
        ...
        if (
            ...
            if (
                val.indexOf('.00') > -1 ||
                parseInt(val.replace(/[^0-9]/g, ''), 10) > 1000
            ) {
                return val.split('.')[0]
            }
        }
